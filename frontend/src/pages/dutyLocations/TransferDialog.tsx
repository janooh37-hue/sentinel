 ...
 * destination unit + post (free-form comboboxes), effective date (default
 * today) and an optional reason. On confirm it POSTs `/duty/transfer`; on
 * success it toasts with a "View record" action that opens `/books/:id`.
 */
...
export function TransferDialog({
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.transferDuty(
        buildTransferRequest({ employeeIds, toUnit, toPost, effectiveDate, reason }),
      ),
    onSuccess: (result) => {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid gap-5 overflow-y-auto px-4 py-4 text-sm md:grid-cols-[1.1fr_1fr]">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor={`${unitListId}-date`} className="text-xs font-semibold text-muted-foreground">
                {t('dutyLocations.transfer.effectiveDate')}
              </label>
              <input
                id={`${unitListId}-date`}
                type="date"
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor={`${unitListId}-reason`} className="text-xs font-semibold text-muted-foreground">
                {t('dutyLocations.transfer.reason')}
              </label>
              <input
                id={`${unitListId}-reason`}
                type="text"
                value={reason}
                dir="auto"
                placeholder={t('dutyLocations.transfer.reasonPlaceholder')}
