...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("version >= 0 AND current_sequence >= 0", name="ck_inmate_violation_workflows_version"),
        sa.CheckConstraint("state IN ('draft','awaiting_review','awaiting_manager','closed')", name="ck_inmate_violation_workflows_state"),
    )
    op.create_index("ix_inmate_violation_workflows_state", "inmate_violation_workflows", ["state"])
    op.create_table(
        "inmate_violation_submissions",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("workflow_id", sa.Integer(), sa.ForeignKey("inmate_violation_workflows.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("workflow_id", sa.Integer(), sa.ForeignKey("inmate_violation_workflows.id", ondelete="CASCADE"), nullable=False),
        sa.Column("submission_id", sa.Integer(), sa.ForeignKey("inmate_violation_submissions.id", ondelete="CASCADE"), nullable=True),
        ...
        sa.CheckConstraint("action IN ('prepared','reviewed','approved','returned','superseded','invalidated','reopened')", name="ck_inmate_violation_actions_kind"),
        sa.CheckConstraint("action NOT IN ('prepared','reviewed','approved') OR (submission_id IS NOT NULL AND actor_user_id IS NOT NULL AND length(trim(actor_name_ar)) > 0 AND actor_name_ar IS NOT NULL AND length(trim(actor_employee_id)) > 0 AND actor_employee_id IS NOT NULL)", name="ck_inmate_violation_actions_actor"),
        sa.CheckConstraint("action NOT IN ('returned','reopened') OR (reason IS NOT NULL AND length(trim(reason)) > 0)", name="ck_inmate_violation_actions_reason"),
    )
    op.create_index("ix_inmate_violation_actions_workflow", "inmate_violation_workflow_actions", ["workflow_id", "id"])
    op.create_index("uq_inmate_violation_actions_stage", "inmate_violation_workflow_actions", ["submission_id", "action"], unique=True, sqlite_where=sa.text("action IN ('prepared','reviewed','approved')"))
    ...
    for period in connection.execute(sa.select(periods).order_by(periods.c.id)).mappings():
        ...
        entries = []
        stored_rows = []
        for stored in connection.execute(sa.select(rows).where(rows.c.period_id == period["id"]).order_by(rows.c.population, rows.c.row_no)).mappings():
            stored_rows.append(dict(stored))
            entry = {key: value for key, value in stored.items() if key not in {"id", "period_id", "row_handle"}}
            ...
            entry.update(manual_row_id=None, missing=[], duplicate_of=None, completion_book_id=None, _sort_created_at="")
            entries.append(entry)
        payload = json.loads(json.dumps({"schema_version": 1, "year": period["year"], "month": period["month"], "entries": entries}, ensure_ascii=False, default=lambda value: value.isoformat()))
        canonical = {**payload, "entries": [{key: value for key, value in row.items() if key not in {"manual_created_by_name", "_sort_created_at"}} for row in payload["entries"]]}
...
def downgrade() -> None:
    ...
    workflows = sa.Table("inmate_violation_workflows", metadata, autoload_with=connection)
    submissions = sa.Table("inmate_violation_submissions", metadata, autoload_with=connection)
    statement = sa.select(periods.c.id, submissions.c.legacy_metadata).join(workflows, (workflows.c.year == periods.c.year) & (workflows.c.month == periods.c.month)).join(submissions, (submissions.c.workflow_id == workflows.c.id) & (submissions.c.sequence == workflows.c.current_sequence)).where(periods.c.closed_at.is_not(None), workflows.c.state == "closed", submissions.c.origin == "legacy")
    for period_id, legacy in connection.execute(statement):
        connection.execute(periods.update().where(periods.c.id == period_id).values(force_reason=(legacy or {}).get("force_reason")))
