...
def resolve_book_pdf(db: Session, book_id: int) -> tuple[str, bytes]:
    ...
    Delegates the artifact selection itself to
    ``document_service.resolve_document_artifact`` — the same
    current-publication interpretation ``documents.download_document`` uses
    (approval-signature-placement plan §8.3), so a placement correction's
    republished package is announced identically to what a browser download
    would now serve, with zero duplicated resolution logic to drift out of
    sync.
