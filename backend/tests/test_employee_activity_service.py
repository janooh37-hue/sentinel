...
def _link(db: Session, entry: LedgerEntry, employee_id: str | None = "G100") -> LedgerEntry:
...
def test_activity_merges_all_sources_by_creation_time(db_session: Session):
    ...
    assert [item.kind for item in result.items] == ["document", "leave", "violation", "ledger"]
...
def test_equal_timestamps_use_kind_then_source_id_tiebreak(db_session: Session):
    ...
    ledger = _ledger(entry_id=44, created_at=BASE)
    db_session.add_all([*documents, leave, violation, ledger])
    _link(db_session, ledger)
    db_session.commit()
...
def test_employee_and_kind_filters_apply_to_rows_and_total(db_session: Session):
    ...
    ledger = _ledger(entry_id=44, created_at=BASE + timedelta(minutes=3), employee_id="G200")
    db_session.add_all(
        [
            Document(
                id=11,
...
def test_global_offset_is_correct_when_one_source_dominates(db_session: Session):
    ...
    ledger = _ledger(entry_id=400, created_at=BASE + timedelta(minutes=3))
    db_session.add_all(
        [
            Leave(
                id=200,
    ...
    pages = [
        employee_activity_service.list_employee_activity(
...
def test_excludes_deleted_drafts_orphans_and_unrelated_rows(db_session: Session):
    _employee(db_session)
    _category_and_book(db_session, book_id=71, ref_number="HR-0071")
    deleted = _ledger(entry_id=83, created_at=BASE, deleted_at=BASE)
    draft = _ledger(entry_id=84, created_at=BASE, tags=["draft"])
    orphan_ledger = _ledger(entry_id=85, created_at=BASE, employee_id=None)
    db_session.add_all(
        [
            Leave(
                id=80,
            ...
            Document(
                ...
                template_id="Draft",
                ref_number="DRAFT",
                docx_path="output/draft.docx",
                submission_id="00000000-0000-0000-0000-000000000081",
                created_at=BASE,
            ),
            Document(
                id=82,
                ...
                template_id="Orphan",
                ref_number="HR-9999",
                docx_path="output/orphan.docx",
                submission_id="00000000-0000-0000-0000-000000000082",
                created_at=BASE,
            ),
            deleted,
            draft,
    ...
    _link(db_session, deleted)
    _link(db_session, draft)
    db_session.commit()
...
def test_private_email_rows_and_total_are_owner_scoped(db_session: Session):
    _employee(db_session)
    owned = _ledger(entry_id=90, created_at=BASE, channel="email", owner_user_id=7)
    other = _ledger(
        entry_id=91, created_at=BASE + timedelta(minutes=1), channel="email", owner_user_id=8
    )
    shared = _ledger(
        entry_id=92, created_at=BASE + timedelta(minutes=2), channel="letter", owner_user_id=8
    )
    db_session.add_all([owned, other, shared])
    _link(db_session, owned)
    _link(db_session, other)
    _link(db_session, shared)
    db_session.commit()
