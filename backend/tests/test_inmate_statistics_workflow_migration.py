...
def test_workflow_migration_is_the_single_sequential_head():
    ...
    assert scripts.get_revision(scripts.get_heads()[0]).down_revision == "0087_vehicle_photo_library"


@pytest.mark.parametrize("state", ["untouched", "reopened", "new_seal"])
def test_legacy_upgrade_and_downgrade_preserve_only_current_force_reason(tmp_path, monkeypatch, state):
    ...
    with engine.begin() as db:
        db.execute(text("INSERT INTO inmate_violation_periods (id,year,month,closed_at,closed_by,force_reason) VALUES (1,2026,8,'2026-09-01 10:00:00',91,'historical reason')"))
        db.execute(text("INSERT INTO inmate_violation_stat_rows (period_id,row_handle,origin,row_no,population,name,violation_date,wing,incomplete_marks) VALUES (1,'manual:7','manual',1,'pending','legacy synthetic','2026-08-02','A1','[\"nationality\"]')"))
        db.execute(text("INSERT INTO inmate_violation_periods (id,year,month,reopened_at,reopened_by) VALUES (2,2026,7,'2026-08-02 10:00:00',92)"))
        db.execute(text("INSERT INTO inmate_violation_stat_rows (period_id,row_handle,origin,row_no,population,name,violation_date) VALUES (2,'manual:8','manual',1,'expats','retained synthetic','2026-07-02')"))
    ...
    with engine.begin() as db:
        legacy = db.execute(text("SELECT payload,legacy_metadata FROM inmate_violation_submissions ORDER BY id")).all()
        assert len(legacy) == 2
        ...
        if state == "reopened":
            ...
            db.execute(text("UPDATE inmate_violation_workflows SET state='draft' WHERE year=2026 AND month=8"))
    if state == "new_seal":
        with Session(engine, autoflush=False, expire_on_commit=False) as db:
            ...
            for index in range(3):
                employee = Employee(id=f"GMIG{index}", name_ar=f"موظف ترحيل تجريبي {index}", name_en="Synthetic")
                db.add(employee)
                db.flush()
                actor = User(email=f"migration{index}@example.test", password_hash="x", role="admin", status="active", employee_id=employee.id)
                db.add(actor)
                actors.append(actor)
            db.commit()
            opened = service.reopen_month(db, 2026, 8, actor=actors[0], expected_version=0, reason="new reviewed report")
            assert opened.workflow["prepared"] is None
            service.create_manual_row(db, 2026, 8, actor=actors[0], name="new synthetic seal", violation_date=date(2026, 8, 2), reason="correct old data", wing="1A", nationality_label="الإمارات", details_text="complete")
            view = service.build_month(db, 2026, 8)
            view = service.prepare_month(db, 2026, 8, actor=actors[0], expected_version=view.workflow["version"], expected_projection_fingerprint=view.projection_fingerprint, reviewer_user_id=actors[1].id)
            view = service.review_month(db, 2026, 8, actor=actors[1], expected_version=view.workflow["version"], submission_id=view.workflow["active_submission_id"], manager_user_id=actors[2].id)
            view = service.approve_month(db, 2026, 8, actor=actors[2], expected_version=view.workflow["version"], submission_id=view.workflow["active_submission_id"], today=date(2026, 9, 12))
    ...
    with engine.connect() as db:
        assert db.scalar(text("SELECT force_reason FROM inmate_violation_periods WHERE id=1")) == ("historical reason" if state == "untouched" else None)
        assert db.scalar(text("SELECT count(*) FROM inmate_violation_stat_rows")) == 2
        assert db.execute(text("SELECT * FROM inmate_violation_stat_rows ORDER BY id")).all() == before
    command.upgrade(config, "0088_inmate_statistics_workflow")
    engine.dispose()
    get_settings.cache_clear()
