...
def test_assigned_pending_approvals_bypass_record_type_denies_but_history_does_not(
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: artifact_service.ArtifactResult(
            docx_path=signed_pdf_path.with_suffix(".docx"),
            conversion=artifact_service.ConversionOutcome(
                status="success", pdf_path=signed_pdf_path
            ),
            created_paths=(signed_pdf_path,),
        ),
    )
    monkeypatch.setattr(
        included_papers_service,
