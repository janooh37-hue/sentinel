...
def _manifest(path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "anchor_count": xml.count(b"<wp:anchor"),
        "runs": [
            [
                {
                    "text": run.text,
                    ...
                    "italic": run.italic,
                    "font": run.font.name,
                    "cs_font": run._r.get_or_add_rPr().rFonts.get(qn("w:cs"))
                    if run._r.get_or_add_rPr().rFonts is not None
                    else None,
                    "rtl": run._r.get_or_add_rPr().find(qn("w:rtl")) is not None,
                }
        ...
        "paragraph_bidi": [
            paragraph._p.get_or_add_pPr().find(qn("w:bidi")) is not None
            for paragraph in document.paragraphs
        ],
        "table_xml": [table._tbl.xml for table in document.tables],
        "section_xml": [section._sectPr.xml for section in document.sections],
...
def test_phase6_windows_word_artifact_smoke() -> None:
    ...
    with SessionLocal() as db:
        ...
        if user is None:
            ...
            db.add(user)
        manager = db.scalar(select(Manager).where(Manager.name_en == "Phase Six Manager"))
        if manager is None:
            manager = Manager(
                name_en="Phase Six Manager",
                name_ar="مدير المرحلة السادسة",
        ...
        leave_manifest = _manifest(leave_docx)
        leave_text = docx_to_text(leave_docx)
        assert "موظف المرحلة السادسة" in leave_text
        assert "مدير المرحلة السادسة" in leave_text
        assert any(leave.ref_number in header for header in leave_manifest["headers"])
        assert int(leave_manifest["drawing_count"]) >= 2  # Aztec plus synthetic signature
        assert any(str(value).strip() for value in leave_manifest["footers"])
        assert leave.ref_number in _pdf_text(leave_pdf)
        ...
        english.bold = True
        arabic_paragraph = authored.add_paragraph()
        arabic_paragraph._p.get_or_add_pPr().append(OxmlElement("w:bidi"))
        arabic = arabic_paragraph.add_run("نص مؤلف للمرحلة السادسة — المرجع P6/2026 والرقم ١٢٣ ABC")
        arabic.italic = True
        arabic.font.name = "Arial"
        arabic._r.get_or_add_rPr().get_or_add_rFonts().set(qn("w:cs"), "Arial")
        arabic._r.get_or_add_rPr().append(OxmlElement("w:rtl"))
        ...
        table.cell(0, 1)._tc.get_or_add_tcPr().append(shading)
        ...
        preview_old_text = _pdf_text(preview_old)
        assert "P6-PREVIEW-OLD" in preview_old_text
        assert "P6-PREVIEW-NEW" not in preview_old_text
        ...
        preview_new_text = _pdf_text(preview_new)
        assert "P6-PREVIEW-NEW" in preview_new_text
        assert _sha256(evidence_dir / "preview-old.pdf") != _sha256(
            evidence_dir / "preview-new.pdf"
        )
        ...
        assert finished_manifest["sha256"] == source_manifest["sha256"]
        for key in (
            "paragraphs",
            "runs",
            "paragraph_bidi",
        ...
        assert "P6-PREVIEW-OLD" in finished_manifest["paragraphs"]
        assert "P6-PREVIEW-NEW" in finished_manifest["paragraphs"]
        ...
        for key in ("tables", "table_xml", "headers", "footers", "section_xml"):
            assert signed_manifest[key] == source_manifest[key]
        for media_name, media_hash in dict(source_manifest["media"]).items():
            assert dict(signed_manifest["media"])[media_name] == media_hash
        assert signed_manifest["drawing_count"] == int(source_manifest["drawing_count"]) + 1
        assert signed_manifest["anchor_count"] == int(source_manifest["anchor_count"]) + 1
        assert "Phase Six Manager" in signed_manifest["paragraphs"]
        shutil.copy2(signed_docx, evidence_dir / "authored-signed.docx")
    ...
    _copy_pdf_pages(evidence_dir / "preview-old.pdf", evidence_dir / "pages", "preview-old")
        ...
        json.dumps(
            {
                ...
                "word_pids": word_pids,
                "dispatch_ex_conversion_sources": _CONVERSION_SOURCES,
                "leave_docx_sha256": _sha256(leave_docx),
                "leave_pdf_sha256": _sha256(leave_pdf),
                "leave_manifest": leave_manifest,
